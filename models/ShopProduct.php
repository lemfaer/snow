<?php
	...
	class ShopProduct {
		...
		private static function checkID($productID) {

			DB::checkedQuery("SELECT id FROM product WHERE id = $productID");
		}
		...
		private static function getColorArray($productID) {
			...
			$query = "SELECT b.id, b.name, b.value 
				FROM size_color AS a INNER JOIN color AS b
				ON a.color_id = b.id
			...
			foreach ($colorArray as $k => $v) {
				$query = "SELECT size_id FROM size_color 
		...
		private static function getSizeArray($productID) {
			...
			$query = "SELECT b.id, b.name
				FROM size_color AS a INNER JOIN size AS b
				ON a.size_id = b.id
				WHERE a.product_id = '$productID' AND a.count > '0' AND b.status = '1'
				GROUP BY b.id
				ORDER BY b.name";
			...
			foreach ($sizeArray as $k => $v) {
				$query = "SELECT color_id FROM size_color 
